...
2026-09-08 按用户测试「影视飓风」（`WORKERS=4`、`COUNT=0`、`QUALITY=best`，整页下载完成后再翻页，页间隔 5 秒）：

- 读完 30 页可访问作品，523 条视频全部下载成功，失败 0 条，另跳过 6 条图文。
- 总耗时 2 小时 2 分 15 秒，整体平均 4.28 条/分钟。
- 日志未记录验证码或下载限流；期间出现过作品列表加载超时，后续恢复并完成。

以上是少量单机样本，并非严格对照或固定触发阈值，也不保证低于上述数量就不验证。用户测试因读完可访问作品而结束，没有测出风控上限；不同采集入口的结果不能直接比较，实测速率也不是安全速度上限。

`WORKERS` 只控制下载并发，不等于元数据请求频率。上述关键词验证发生在搜索翻页阶段；历史关键词测试按页下载，当前关键词入口先搜索再下载，不能将历史数量直接作为运行上限。
